...
int chk_blockdomain(const char * domain_str);
...
int chk_blockdomain(const char* domain_str)
{
	MYSQL* mysql;
        ...
        mysql_real_connect(mysql, "127.0.0.1", "root", "1234", "domaindata", 3306, NULL, 0);
        
        char query[100] = "select * from blocklist;";
        mysql_real_query(mysql, query, (unsigned long)strlen(query));
        
	MYSQL_RES* result = mysql_store_result(mysql);
	if(result == NULL) printf("result error\n");
	...
	while(row = mysql_fetch_row(result))
	{
		...
		printf("check blocked domain list %d : %s\n",i, row[0]);
		i++;
		if(strcmp(domain_str, row[0]) == 0)
		{
...
void print_domain_name(const u_char* packet)
{
	...
	u_char *domain = NULL;
        ...
        */
		...
		//save domain name to DB
		save_domainname(domain_str, packet);
...
void save_domainname(const char* domain_str, const u_char* packet)
{
	...
	MYSQL* mysql;
        ...
        char query1[100];
        sprintf(query1, "insert into domainlist values('%s', '%s')", domain_str, IPbuffer2_str);
        
        //printf("%s\n", query1);
                
        mysql_send_query(mysql, query1, (unsigned long)strlen(query1));
