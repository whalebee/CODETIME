...
int cnt = 1; // block list count
#define DOMAIN_BUF 260
#define REC_DOM_MAX 20
#define REC_DOM_LEN 260
char domain_arr[REC_DOM_MAX][REC_DOM_LEN] = {0x00};
...
void mysql_get_block_list();
void mysql_compare_block_list(u_char* domain_str, const u_char* packet);
...
int main(int argc, char *argv[])
{
    ...
    // block list : get block list
    mysql_get_block_list();
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) 
{
	...
	if(domain_len){
		...
		mysql_compare_block_list(domain_str, packet);
...
void mysql_get_block_list(){
    // Receive tb_packet_block---------------------------------
		res_block = mysql_perform_query(connection, "SELECT * FROM tb_block_domain_list");
		// domain_arr = {0x00}; // domain_arr array for print block_list
		// REC_DOM_MAX 20
		// REC_DOM_LEN 1024
		int num = 0;
		...
		while( (row_block = mysql_fetch_row(res_block) ) != NULL){
			printf("Mysql block_list in tb_block_domain_list [ row : %d | ID : %s ] \n", cnt++, row_block[0]);
			printf("domain: %20s | ", row_block[1]);
            strcpy( &domain_arr[num++][0], row_block[1]); 			
			printf("comment: %5s | \n", row_block[2]);
        }
