...
int sendraw( u_char* pre_packet, int mode)
{
	...
	// relevented at socket variables
	int raw_socket, recv_socket;
	int on=1, len ;
	char recv_packet[100], compare[100];
	
	
	struct iphdr *iphdr;				// final IP header
	struct tcphdr *tcphdr;				// final TCP header
	struct in_addr source_address, dest_address;	// temporarily saved src.addr and dst.addr
	struct sockaddr_in address, target_addr;
	struct pseudohdr *pseudo_header;
	struct in_addr ip;
	struct hostent *target;
	
	int port;					// port is already '80'
	...
	int ret = 0 ;
		...
		if ( warning_page == 5 ){
			...
			post_payload_size = 230 + 92  ;   // Content-Length: header is changed so post_payload_size is increased.
