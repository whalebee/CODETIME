...
int main(int argc, char *argv[])
{
        ...
        MYSQL * conect;
        char * server = "192.168.111.100"; //dbserver ip
        ...
        mysql_real_connect(conect, server, user, password, database, 0, NULL, 0);
...
int result = 0 ;
	...
	char query_buffer[1024];
        ...
        snprintf(query_buffer, sizeof(query_buffer),
                "INSERT web ( ip_dst,tcp_dst_port, domain_name) "
                "VALUES ( '%s', %u, '%s')",
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
 ...
 char IPbuffer2_str[16];
        ...
        if (mysql_send_query(conect, query_buffer, strlen(query_buffer)) != 0) {
                fprintf(stderr, "query send error: %s\n", mysql_error(conect));
        }
